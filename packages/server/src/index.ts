...
const b = procedure
  .middleware(async (input, ctx, next) => {
    if (true as boolean) {
      return new RPCError({ code: "FAILED IN B" });
    }
    return next(input, ctx);
  })
  .query(async () => {});

const test = procedure
  .schema((input) => {
    if (typeof input === "object") {
      return input;
    }
    return new RPCError({ code: "INVALID_INPUT" });
  })
  .middleware(async (input, ctx, next) => {
    if (!ctx) {
      return new RPCError({
    ...
    if (true as boolean) {
      return new RPCError({ status: 400, code: "ANOTHER_FAIL" } as const);
    }
  ...
  .query(async () => {
    ...
    if (true as boolean) {
      return new RPCError({ code: "YET_ANOTHER_FAIL" });
    }
    return "hello";
  });
